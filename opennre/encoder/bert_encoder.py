...
from .kebiolm.modeling_kebio import KebioModel
from .kebiolm.configuration_kebio import KebioConfig
...
class BERTEncoder(nn.Module):
    def __init__(self, max_length, pretrain_path, blank_padding=True, mask_entity=False):
        ...
        if pretrain_path.find('kebiolm') == -1:
            logging.info('Loading BERT pre-trained checkpoint.')
            self.bert = BertModel.from_pretrained(pretrain_path)
            self.tokenizer = BertTokenizer.from_pretrained(pretrain_path)
        else:
            logging.info('Loading KebioLM')
            config = KebioConfig.from_pretrained(pretrain_path)
            self.bert = KebioModel.from_pretrained(pretrain_path, config=config)
            self.tokenizer = BertTokenizer.from_pretrained(pretrain_path)
...
class BERTEntityEncoder(nn.Module):
    ...
    def forward(self, token, att_mask, pos1, pos2):
        ...
        """
        hidden = self.bert(token, attention_mask=att_mask, return_dict=False)
        if self.pretrain_path.find('kebiolm') == -1:
            hidden = hidden[0]
        else:
            hidden = hidden[2]
...
class BERTEntityEncoderCODER(nn.Module):
    def __init__(self, max_length, pretrain_path, pretrain_path_coder,
        ...
        self.mask_entity = mask_entity
        self.pretrain_path = pretrain_path
        self.pretrain_path_coder = pretrain_path_coder
        if pretrain_path.find('kebiolm') == -1:
            logging.info('Loading BERT pre-trained checkpoint.')
            self.bert = BertModel.from_pretrained(pretrain_path)
            self.tokenizer = BertTokenizer.from_pretrained(pretrain_path)
            ...
            self.bert = KebioModel.from_pretrained(pretrain_path, config=config)
            self.tokenizer = BertTokenizer.from_pretrained(pretrain_path)
        self.coder = BertModel.from_pretrained(pretrain_path_coder)
        self.coder_tokenizer = BertTokenizer.from_pretrained(pretrain_path_coder)
