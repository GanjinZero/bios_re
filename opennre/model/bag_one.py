...
class NewBagOne(BagOne):
    ...
    def infer(self, bag):
        ...
        mask2s = []
        for item in bag:
            token, pos1, pos2, mask, ent1, ent2, mask1, mask2 = self.sentence_encoder.tokenize(item)
            tokens.append(token)
            pos1s.append(pos1)
            pos2s.append(pos2)
            masks.append(mask)
        ...
        pos2s = torch.cat(pos2s, 0).unsqueeze(0)
        masks = torch.cat(masks, 0).unsqueeze(0)
        ent1s = torch.cat(ent1s, 0).unsqueeze(0)
        ent2s = torch.cat(ent2s, 0).unsqueeze(0)
        mask1s = torch.cat(mask1s, 0).unsqueeze(0)
        mask2s = torch.cat(mask2s, 0).unsqueeze(0)
        scope = torch.tensor([[0, len(bag)]]).long() # (1, 2)
        bag_logits = self.forward(None, scope, tokens, pos1s, pos2s, masks, 
                                  ent1s, ent2s, mask1s, mask2s,
                                  train=False).squeeze(0) # (N) after softmax
        score, pred = bag_logits.max(0) # TODO: check multi label
        score = score.item()
        pred = pred.item()
        rel = self.id2rel[pred]
        return (rel, score)
